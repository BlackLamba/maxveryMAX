...
Дополнительно (не забыть) — по итогам разбора архитектуры;

Общее:

 Docker-инфраструктура целиком: Dockerfile под каждый компонент, compose.yaml, .dockerignore, .env.example, сборка ≤5 минут — обязательная часть формата сдачи, сейчас висит без имени. Предлагается закрепить за Женей (ближе всех к БД/инфре) либо явно зарезервировать отдельное время в конце всей командой.
 README.md по чек-листу из требований трека: назначение решения, сценарий проверки, переменные окружения, порты, зависимости, порядок работы с тестовыми данными, известные ограничения — тоже сейчас без владельца.;

Женя (бэк):

 POST /api/events/{id}/click (или аналог) — лог перехода пользователя на внешний источник. Без этого нечем измерить метрику «доля пользователей, дошедших до перехода» (см. 08_пилотный_запуск.md). Маленькая задача, но без неё пилот нечем будет оценивать.
 Явно пометить POST /api/search/natural, favorites, preferences как не Must Have (это Should/Could по 05_MVP.md) — чтобы не тратить первый день на них в ущерб базовому GET /api/events с фильтрами, без которого сценарий вообще не собирается end-to-end.;

Олежа (воркер):

 Пересмотреть порядок: сначала рабочий import JSON → БД (разблокирует Женю и Диму), только потом — дедупликация (RapidFuzz), quality_score, scheduler с retry и юнит-тесты. Текущий список довольно амбициозен для MVP, где по 05_MVP.md вполне достаточно подготовленного тестового набора без реальной интеграции.;

Антон + Дима (бот и фронт вместе):

 Перед синхронизацией формата deep link (?city=...&date=...&category=...&price_max=...) свериться с актуальной документацией MAX — организаторы прямо предупреждают, что API мини-приложений меняется, и примеры из старых источников могут не работать.;
